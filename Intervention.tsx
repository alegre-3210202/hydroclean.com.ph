...
export default function Intervention() {
    ...
    useEffect(() => {
        document.title = 'Intervention - HydroClean';
    }, []);
    ...
    const [hazardousMessages, setHazardousMessages] = useState<string[]>([]); // State to hold hazardous messages
    ...
    const isHazardous = (item) => {
        const severeDanger = 
            (item.turbidity >= 5 && item.turbidity <= 3000) ||
            (item.ph_level >= 0 && item.ph_level <= 1.99) ||
            (item.ph_level >= 14) ||
            (item.ph_level >= 2.00 && item.ph_level <= 5.99) ||
            (item.tdsValue >= 301 && item.tdsValue <= 3000) ||
            (item.temperature_level > 40.1) ||
            (item.temperature_level >= 0 && item.temperature_level <= 4.99) ||
            (item.temperature_level >= 34.00 && item.temperature_level <= 40.00);
    
        const moderateDanger = 
            (item.turbidity >= 5 && item.turbidity <= 999) ||
            (item.ph_level >= 2.00 && item.ph_level <= 5.99) ||
            (item.ph_level >= 9.01 && item.ph_level <= 13) ||
            (item.tdsValue >= 301 && item.tdsValue <= 1200) ||
        ...
        return severeDanger || moderateDanger;
    };
            ...
            const fetchData = async () => {
                ...
                try {
                    ...
                    if (Array.isArray(result)) {
                        const validData = result.map((item) => ({
                        ...
                        })).filter((item) => isValidDate(item.dateTime) && isHazardous(item));
    ...
    const hazardousData = sortedData.filter(item => isHazardous(item));

    const getHazardousParameterData = () => {
        const hazardousParameters = hazardousData.map(item => {
            let intervention = ''; // Define intervention message
    
            if (item.turbidity > 5) {
                intervention = 'Turbidity is high, filter the water to ensure it remains safe for use.';
                return { dateTime: item.dateTime, value: item.turbidity, label: 'Turbidity', intervention };
            }
            if (item.ph_level >= 2.00 || item.ph_level <= 5.99) {
                intervention = 'Adjust pH to neutral range to ensure water is safe for consumption.';
                return { dateTime: item.dateTime, value: item.ph_level, label: 'pH', intervention };
            }
            if (item.ph_level >= 0 || item.ph_level <= 1.99) {
                intervention = 'Adjust pH to neutral range to ensure water is safe for consumption.';
                return { dateTime: item.dateTime, value: item.ph_level, label: 'pH', intervention };
            }
            if (item.ph_level >= 14) {
            ...
            if (item.tdsValue >= 301 || item.ph_level <= 3000) {
                intervention = 'Reduce TDS levels to meet safe drinking standards.';
                return { dateTime: item.dateTime, value: item.tdsValue, label: 'TDS', intervention };
            }
            if (item.temperature_level >= 40.1) {
                intervention = 'Lower water temperature to prevent bacterial growth.';
                return { dateTime: item.dateTime, value: item.temperature_level, label: 'Temperature', intervention };
            }
            if (item.temperature_level >= 0 || 4.99) {
                intervention = 'Temperature is too low, warm it up slightly to help prevent bacteria from growing.';
                return { dateTime: item.dateTime, value: item.temperature_level, label: 'Temperature', intervention };
            }
            if (item.temperature_level >= 34.00 || 40.00) {
                intervention = 'Lower water temperature to prevent bacterial growth.';
                return { dateTime: item.dateTime, value: item.temperature_level, label: 'Temperature', intervention };
            }
            return null; // Return null if no hazardous parameter is found
        }).filter(param => param !== null); // Filter out null values
    
        return hazardousParameters;
    };
    
    const hazardousParameterData = getHazardousParameterData();

    const lineChartData = {
        labels: hazardousData.map(item => 
        ...
        datasets: [
            {
                label: 'Turbidity',
                data: hazardousData.map(item => item.turbidity),
                borderColor: '#b30086',
                fill: false,
                pointStyle: hazardousData.map(() => 'circle'), // Keep as circle
                pointBackgroundColor: hazardousData.map(item => 
                    item.turbidity >= 5 ? 'red' : 'rgba(255, 255, 255, 0)'),
            },
            {
                label: 'TDS',
                data: hazardousData.map(item => item.tdsValue),
                borderColor: '#FFD700',
                fill: false,
                pointStyle: hazardousData.map(() => 'circle'), // Keep as circle
                pointBackgroundColor: hazardousData.map(item => 
                    item.tdsValue >= 301 && item.tdsValue <= 3000 ? 'red' : 'rgba(255, 255, 255, 0)')
            },
            {
                label: 'pH',
                data: hazardousData.map(item => item.ph_level),
                borderColor: '#7398d0',
                fill: false,
                pointStyle: hazardousData.map(() => 'circle'), // Keep as circle
                pointBackgroundColor: hazardousData.map(item => 
                    ...
                    (item.ph_level >= 9.01 && item.ph_level <= 13) || 
                    (item.ph_level >= 0 && item.ph_level <= 1.99) || 
                    item.ph_level >= 14 ? 'red' : 'rgba(255, 255, 255, 0)')
            },
            {
                label: 'Temperature',
                data: hazardousData.map(item => item.temperature_level),
                borderColor: '#ffa500',
                fill: false,
                pointStyle: hazardousData.map(() => 'circle'), // Keep as circle
                pointBackgroundColor: hazardousData.map(item => 
                    (item.temperature_level >= 0 && item.temperature_level <= 4.99) || 
                    (item.temperature_level >= 34.00 && item.temperature_level <= 40.00) || 
                    item.temperature_level > 40.1 ? 'red' : 'rgba(255, 255, 255, 0)')
            },
        ],
    };
    ...
    const printToPDF = () => {
        ...
        pdf.setFontSize(20);
        pdf.text('HydroClean Intervention Report', 20, 130); // Adjusted Y position after image
        pdf.setFontSize(14);
        pdf.text(`Data from ${startDate} to ${endDate}`, 20, 160); // Adjusted Y position after title
    
        // Create a table header
        const headers = [["Date & Time", "Parameter", "Value", "Intervention"]];
        
        // Prepare the data for the PDF
        const pdfData = hazardousParameterData.map(param => [
            // Format date and time similar to the provided format
            new Date(param.dateTime).toLocaleString([], { 
        ...
        pdf.save(`HydroClean_InterventionReport_${startDate}_${endDate}.pdf`);
    };
        ...
        <>
            ...
            <div className={localStorage.getItem('isDarkMode') === 'true' ? 'container dark-mode' : 'container'}>
                ...
                <Sidebar items={[
                    ...
                    { icon: 'report_gmailerrorred', label: 'Intervention', href: 'intervention', active: true  },
 ...
 <button onClick={printToPDF} className="print-button custom-button"
    ...
    <table>
        ...
        <tbody>
        {hazardousParameterData.map((param, index) => (
                <tr key={index}>
                    <td>{new Date(param.dateTime).toLocaleString([], { 
                        month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit', hour12: true 
                    })}</td>
                    <td>{param.label}</td>
                    <td>{param.value}</td>
                    <td>{param.intervention}</td>
                </tr>
            ))}
        </tbody>
    </table>
                    ...
                    <h2>Hazardous Parameters and Intervention</h2>

    
                    <div className={localStorage.getItem('isDarkMode') === 'true' ? 'intervention-table-container dark-mode' : 'intervention-table-container'}>
